    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
          ...
          <button
            onClick={closeCart}
            className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors cursor-pointer text-nearblack"
              ...
              return (
                <div key={`${item.product.id}-${item.size}-${item.color.name}`} className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-nearblack leading-snug" style={{ fontFamily: "var(--font-serif)" }}>
                      {item.product.name}
                    </p>
                    ...
                    <p className="text-sm font-bold text-maroon mt-1.5">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                        <button
                          ...
                          className="w-7 h-7 flex items-center justify-center hover:bg-gray-100 text-sm cursor-pointer text-nearblack"
                          aria-label="Decrease quantity"
                        >−</button>
                        <span className="w-7 text-center text-sm font-medium text-nearblack">{item.quantity}</span>
                        <button
                          onClick={() => updateQuantity(item.product.id, item.size, item.color.name, item.quantity + 1)}
                          className="w-7 h-7 flex items-center justify-center hover:bg-gray-100 text-sm cursor-pointer text-nearblack"
                      ...
                      <button
                        ...
                        className="text-xs text-muted hover:text-maroon transition-colors cursor-pointer"
                      >
                        Remove
                      </button>
                    </div>
