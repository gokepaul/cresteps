    ...
    <nav className="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <Link
            href="/"
            className="text-2xl font-bold tracking-widest text-maroon"
          ...
          <div className="hidden md:flex items-center gap-6">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-sm font-medium text-nearblack hover:text-maroon transition-colors"
              >
                {link.label}
              </Link>
            ))}
          ...
          >
            <span
              className={`block h-0.5 w-6 bg-nearblack transition-all duration-300 ${menuOpen ? "rotate-45 translate-y-2" : ""}`}
            />
            <span
              className={`block h-0.5 w-6 bg-nearblack transition-all duration-300 ${menuOpen ? "opacity-0" : ""}`}
            />
            <span
              className={`block h-0.5 w-6 bg-nearblack transition-all duration-300 ${menuOpen ? "-rotate-45 -translate-y-2" : ""}`}
            />
